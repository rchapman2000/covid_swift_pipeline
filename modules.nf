...
process GenerateConsensus {
    ...
    input:
        ...
        file CORRECT_OVERLAPPING_VARIANTS
    ...
    then
        ...
        /usr/local/miniconda/bin/bcftools sort \${R1}_pre1.vcf | /usr/local/miniconda/bin/bcftools filter -i '(DP4[0]+DP4[1]) < (DP4[2]+DP4[3]) && ((DP4[2]+DP4[3]) > 0)' --threads !{task.cpus} | /usr/local/miniconda/bin/bcftools filter -e 'IMF < 0.5' -o \${R1}_pre2.vcf
        #/usr/local/miniconda/bin/bcftools filter -e 'IMF < 0.5' \${R1}_pre2.vcf -o \${R1}.vcf

        # Corrects overlapping variants by picking the higher AD
        python3 !{CORRECT_OVERLAPPING_VARIANTS} -file \${R1}_pre2.vcf
